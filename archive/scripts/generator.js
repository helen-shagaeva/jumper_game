function Generator(width_, height_) {
    ...
    this.slatsGenerate = function (slats, need_height, first) {
        ...
        while (tmp_ypos > need_height) {       // путь
            ...
            if ((current_slat.ypos - tmp_ypos) > 240) {
                current_slat.ypos = current_slat.ypos - ((current_slat.ypos - tmp_ypos) - 240);
            }

            var direction = this.getRandomInt(0, 10);
            if ((current_slat.xpos + 50) > (this.width - 10)) {
                direction = 8;
            }   // to left
            ...
            if (direction < 5) {    // right
                ...
                if (current_slat.xpos < 0) {
                    current_slat.xpos = current_slat.xpos * (-1);
                }
            }
